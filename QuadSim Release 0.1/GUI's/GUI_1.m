...
% Last Modified by GUIDE v2.5 16-Feb-2014 15:43:14
...
imshow('Motors_Graphic.jpg'); % Displays default graphic when GUI first opens  
...
set(handles.HUB_L_unit, 'String', 'cm');
...
function HUB_L_edit_Callback(hObject, eventdata, handles)
% hObject    handle to HUB_L_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of HUB_L_edit as text
%        str2double(get(hObject,'String')) returns contents of HUB_L_edit as a double


% --- Executes during object creation, after setting all properties.
function HUB_L_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to HUB_L_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
if massUnits == 'g',
    ...
    HUB_L_cm = str2num(get(handles.HUB_L_edit, 'String'));
    ...
    HUB_L_in = str2num(get(handles.HUB_L_edit, 'String'));
...
quadModel = struct('g',(g),'d',(d),'mass',(mass),'ct',(ct),'cq',(cq),'Jx',(Jx),'Jy',(Jy),'Jz',(Jz),'Jb',(Jb),'Jbinv',(Jbinv),'dctcq',(dctcq),...
    'motor_m',(motor_m),'motor_dm',(motor_dm),'motor_h',(motor_h),'motor_r',(motor_r),...
    'ESC_m',(ESC_m),'ESC_a',(ESC_a),'ESC_b',(ESC_b),'ESC_ds',(ESC_ds),...
    'HUB_m',(HUB_m),'HUB_r',(HUB_r),'HUB_L',(HUB_L),...
    'arms_m',(arms_m),'arms_r',(arms_r),'arms_L',(arms_L),'T',(T),'minThr',(minThr),...
    'cr',(cr),'b',(b));

% Saves the structure "quadModel" into the working directory
uisave('quadModel','quadModel');
...
if massUnits == 'g',
    ...
    Lh_cm = str2num(get(handles.HUB_L_edit,'String')); 

    mh = mh_g/1000; % Converts g to kg
    rh = rh_cm/100; % Converts cm to m
    Lh = Lh_cm/100; % Converts cm to m
    
    % Jx3, Jy3, Jz3 --- Inertia's about each axis for Central HUB only
    Jx3 = (1/4)*((mh))*((rh)^2) + (1/12)*((mh))*((Lh)^2);
    Jy3 = (1/4)*((mh))*((rh)^2) + (1/12)*((mh))*((Lh)^2);
    Jz3 = (1/2)*((mh))*((rh)^2);
    ...
    Jx4 = ((3/2)*(ma)*(ra^2)) + ((7/6)*(ma)*(La^2));
    Jy4 = ((3/2)*(ma)*(ra^2)) + ((7/6)*(ma)*(La^2));
    Jz4 = ((ma)*((ra)^2)) + (7/3)*((ma) * ((La)^2));
    ...
    % Automatically calculating the Cq value - based on Ct
 
    % First get the Ct value from the GUI input
    ctVal = str2num(get(handles.ct_edit,'String'));

    % Relation based on Ct given below:
    cqVal = (ctVal)*sqrt(ctVal/2);

    % Set the Cq display handles to the calculated Cq value
    set(handles.cq_edit,'String',cqVal);
    ...
    Lh_in = str2num(get(handles.HUB_L_edit,'String')); 

    mh = mh_oz/35.274; % Converts oz to kg
    rh = rh_in/39.3701; % Converts in to m
    Lh = Lh_in/39.3701; % Converts in to m
...
% This "Retrieve" button is used when the user wishes to change only a few
...
set(handles.HUB_L_edit,'String',quadModel.HUB_L*100);
...
switch get(eventdata.NewValue, 'Tag')
    ...
    case 'eng_button' % English units
        ...
        set(handles.HUB_L_unit, 'String', 'in');
